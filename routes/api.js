...
router.post('/all', async (req, res) => {
    ...
    const timetableTomorrow = await edupage.getTimetableForDate(dateTomorrow);
    const gradesRaw = edupage.grades.reverse();
    ...
    const timeline = parseTimeline(edupage.timelineItems, 25);
...
router.post('/timeline/:amount?', async (req, res) => {
    const username = req.body.username;
    const password = req.body.password;
    ...
    try {
        ...
        const timeline = parseTimeline(edupage.timelineItems, amount);
...
process.on('uncaughtException', (error) => { // This should not be here but due to how EdupageAPI handles LoginErrors, it has to be here
